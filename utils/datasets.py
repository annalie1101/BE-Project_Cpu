...
def transform_img (img0, img_size = 640, stride = 32):
    '''Works as LoadImages but we just need to read from the img matrix, not the path
    Returns:
        img (np.ndarray): transformed image
        img0 (cv2) : original image
    '''
    # Padded resize
...
class LoadImages:  # for inference
    '''args:
    - path: path have images/videos
    - img - size
    returns:
    - path
    img: transformed img (resize, padding, RGB)
    img0: original image
    self.cap
    '''
...
def letterbox(img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    new_unpad = int(round(shape[1] * r)), int(round(shape[0] * r)) # new shape without padding
